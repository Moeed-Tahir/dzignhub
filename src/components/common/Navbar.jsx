          ...
          {menuItems.map((item) => (
            <div key={item.name} className="relative">
              <button
                ...
                className={`relative px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 flex items-center space-x-2 font-semibold text-[#202126] ${activeMenu === item.name
                  ? 'text-black px-15'
                  : ''
                  }`}
                style={{
                    ...
                    <div key={notification.id} className={`px-4 py-3 hover:bg-gray-50`}>
                      <div className="flex items-center justify-between">
                        <div className="mr-3 bg-[#C209C11A] rounded-full">
                          <Image src={notification.icon} alt="Notification Icon" width={64} height={64} className="w-10 h-10 p-2" />
                          {/* {notification.icon} */}
                        </div>
